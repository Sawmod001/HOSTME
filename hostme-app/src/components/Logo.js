...
export default function Logo({ size = "md", showTagline = false, href = "/" }) {
  const sizes = {
  ...
  const content = (
    <div className="flex items-center gap-2.5">
      ...
      <div className="flex flex-col">
        <span className={`${s.text} font-bold tracking-tight`} style={{ color: "var(--color-ink)" }}>
          Host<span style={{ color: "var(--color-primary)" }}>Me</span>
        </span>
        {showTagline && <span className={`${s.tagline} leading-tight`} style={{ color: "var(--color-ink-muted)" }}>Find your space</span>}
      </div>
    </div>
  );
